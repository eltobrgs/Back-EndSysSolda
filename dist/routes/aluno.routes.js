...
alunoRouter.use(auth_1.authMiddleware);
...
alunoRouter.post('/', async (req, res) => {
    try {
        const { nome, cpf, email, telefone, idade, usaOculos, destroCanhoto, cursoId, modulos, } = req.body;
...
alunoRouter.put('/:id/progresso', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (modulosStatus && modulosStatus.length > 0) {
            await Promise.all(modulosStatus.map(async (modulo) => {
